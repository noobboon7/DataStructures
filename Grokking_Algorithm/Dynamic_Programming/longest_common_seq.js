...
const longestSubstring = (firstWord = "", secondWord = "") => {
	const matrix = JSON.parse(JSON.stringify(createMatrix(firstWord.length, secondWord.length)));
	let sizeSequence = 0;
	let indexSequence = 0;
...
const longestCommonSubsequence = (firstWord = "", secondWord = "") => {
	...
	for (let i = 0; i < firstWord.length; i++) {
		for (let j = 0; j < secondWord.length; j++) {
			if (firstWord[i] === secondWord[j]) {
        // current cell = (if i or j not 0), then use top left cell value + 1 : else 1
				matrix[i][j] = (i && j) > 0 ? matrix[i - 1][j - 1] + 1 : 1;
			} else {
        // i > 0 && j > 0 check for array bounds
        // current cell = choice of higher number between cell atop current cell or cell left of current cell
				matrix[i][j] = Math.max(i > 0 ? matrix[i - 1][j] : 0, j > 0 ? matrix[i][j - 1] : 0);
			}
		}
	}
...
console.log(
longestCommonSubsequence("blue", "clues") // 3
)